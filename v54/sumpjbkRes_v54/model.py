...
def _cat1_logits_proj(logits):
   with tf.variable_scope('cat_1'):
      ...
      logit_2 = exp2
      logit_3 = _logsumexp(tf.concat([exp3, exp4, exp5, exp7], axis=1))
      logit_4 = exp6
      logit_5 = exp8
      logits = tf.concat([logit_0, logit_1, logit_2, logit_3, logit_3, logit_3, logit_4, logit_3, logit_5, logit_1], axis=1) 
   return logits
...
def _cat2_logits_proj(logits):
   with tf.variable_scope('cat_2'):
      ...
      logits = tf.concat([logit_0, logit_0, logit_1, logit_1, logit_1, logit_1, logit_1, logit_1, logit_0, logit_0], axis=1)
   return logits
...
def network(net, labels):
   ...
   net = _residual(net, 64, 64, 'unit_16_3')
   net = _residual(net, 64, 64, 'unit_16_4')
   net = _residual(net, 64, 64, 'unit_16_5')
   net = slim.layers.max_pool2d(net, [2,2], scope='pool_1')

   net = _residual(net, 64, 128, 'unit_32_1')
   net = _residual(net, 128, 128, 'unit_32_2')
   net = _residual(net, 128, 128, 'unit_32_3')
   net = _residual(net, 128, 128, 'unit_32_4')
   net = _residual(net, 128, 128, 'unit_32_5')
   net = slim.layers.max_pool2d(net, [2,2], scope='pool_2')

   net = _residual(net, 128, 256, 'unit_64_1')
   net = _residual(net, 256, 256, 'unit_64_2')
   net = _residual(net, 256, 256, 'unit_64_3')
   net = _residual(net, 256, 256, 'unit_64_4')
   net = _residual(net, 256, 256, 'unit_64_5')
   ...
   logits_cat1 = _cat1_logits_proj(logits)
   logits_cat2 = _cat2_logits_proj(logits)
