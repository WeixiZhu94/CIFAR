...
flags.DEFINE_string('log_dir', '../log_cifar10_conv/sum_v14/train',
...
def main(train_dir, batch_size, num_batches, log_dir, lrn):
    ...
    total_loss = loss + tf.where(tf.less(loss_cat1, -tf.log(0.5)), loss_cat1, 0) + tf.where(tf.less(loss_cat2, -tf.log(0.5)), loss_cat2, 0)
